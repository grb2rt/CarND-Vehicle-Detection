...
folder = x[0:10]+"_"+x[11:13]+"_"+x[15:16]
os.makedirs(folder)
...
#    '''	# for debugging:
    ...
    cv2.putText(window_img, "hist_bins: "+str(hist_bins) , (100, 100), font, 1, (0, 255, 255), 2, cv2.LINE_AA)
    cv2.imwrite(folder+'/hist/hist_'+str(nnn)+'.jpg', window_img)

    window_img = lesson_functions.draw_boxes(draw_image, hot_windows_hog , color=(255, 255, 0), thick=2)                    
    cv2.putText(window_img, str(nnn) , (50, 100), font, 1.5, (255, 255, 255), 2, cv2.LINE_AA)
    cv2.putText(window_img, "hot windows found: "+str(len(hot_windows_hog)) , (100, 100), font, 1.5, (255, 255, 0), 2, cv2.LINE_AA)
    ...
    window_img = lesson_functions.draw_boxes(draw_image, hot_windows, color=(0, 0, 255), thick=2)                    
    cv2.putText(window_img, str(nnn) , (50, 100), font, 1.5, (255, 255, 255), 2, cv2.LINE_AA)
    cv2.putText(window_img, "hot windows found: "+str(len(hot_windows)) , (100, 100), font, 1.5, (0, 0, 255), 2, cv2.LINE_AA)
    cv2.putText(window_img, "abs_threshold: "+str(abs_threshold) + "  dyn_lim: "+str(dyn_lim)+ " = "+str(int(dyn_lim*len(hot_windows))) , (100, 100), font, 1.5, (0, 0, 255), 2, cv2.LINE_AA)
    cv2.imwrite(folder+'/all/all_'+str(nnn)+'.jpg', window_img)
    # document single image output
#    '''
    global nnn
    nnn = nnn + 1
...
clip1 = VideoFileClip("input_video/project_video.mp4").subclip(1, 2)
