...
dyn_lim = 0.12 # percentage of detections to locate object
abs_threshold = 30 # no of min detections to detect object
...
labels = heatmap.heathot(hot_windows, test_image, dyn_threshold, abs_threshold)
...
#    '''	# for debugging:
    ...
    cv2.putText(window_img, "hot windows found: "+str(len(hot_windows_spat)) , (100, 100), font, 1.5, (255, 0, 255), 2, cv2.LINE_AA)
    cv2.putText(window_img, "spatial bins: "+str(spatial_size) , (100, 200), font, 1, (255, 0, 255), 2, cv2.LINE_AA)
    cv2.imwrite(folder+'/spat/spat_'+str(nnn)+'.jpg', window_img)

    window_img = lesson_functions.draw_boxes(draw_image, hot_windows_hist, color=(0, 255, 255), thick=2)                    
    cv2.putText(window_img, str(nnn) , (50, 100), font, 1.5, (255, 255, 255), 2, cv2.LINE_AA)
    cv2.putText(window_img, "hot windows found: "+str(len(hot_windows_hist)) , (100, 100), font, 1.5, (0, 255, 255), 2, cv2.LINE_AA)
    cv2.putText(window_img, "hist_bins: "+str(hist_bins) , (100, 200), font, 1, (0, 255, 255), 2, cv2.LINE_AA)
    cv2.imwrite(folder+'/hist/hist_'+str(nnn)+'.jpg', window_img)

    window_img = lesson_functions.draw_boxes(draw_image, hot_windows_hog , color=(255, 255, 0), thick=2)                    
    cv2.putText(window_img, str(nnn) , (50, 100), font, 1.5, (255, 255, 255), 2, cv2.LINE_AA)
    cv2.putText(window_img, "hot windows found: "+str(len(hot_windows_hog)) , (100, 100), font, 1, (255, 255, 0), 2, cv2.LINE_AA)
    cv2.putText(window_img, "orient: "+str(orient)+"  pix_per_cell: "+str(pix_per_cell)+"  cell_per_block: "+str(cell_per_block)+"  hog_channel: "+str(hog_channel) , (100, 200), font, 1, (255, 255, 0), 2, cv2.LINE_AA)
    cv2.imwrite(folder+'/hog/hog_'+str(nnn)+'.jpg', window_img)

    window_img = lesson_functions.draw_boxes(draw_image, hot_windows, color=(0, 0, 255), thick=2)                    
    cv2.putText(window_img, str(nnn) , (50, 100), font, 1.5, (255, 255, 255), 2, cv2.LINE_AA)
    cv2.putText(window_img, "hot windows found: "+str(len(hot_windows)) , (100, 100), font, 1, (0, 0, 255), 2, cv2.LINE_AA)
    cv2.putText(window_img, "abs_threshold: "+str(abs_threshold) + "  dyn_lim: "+str(dyn_lim)+ " = "+str(int(dyn_lim*len(hot_windows))) , (100, 200), font, 1, (0, 0, 255), 2, cv2.LINE_AA)
    cv2.putText(window_img, "spat: "+str(len(hot_windows_spat))+"  hist: "+str(len(hot_windows_hist))+"  hog: "+str(len(hot_windows_hog)) , (100, 300), font, 1, (0, 0, 255), 2, cv2.LINE_AA)
    # document single image output
#    '''
...
#    dyn_threshold = int(burning_windows*0.15) # % of all frames must be overlapping object for detection
    ...
    labels = heatmap.heathot(hot_windows, draw_image, dyn_threshold, abs_threshold)
...
#    '''
    ...
    cv2.putText(draw_img, "hot windows found: "+str(len(hot_windows_hist)) , (100, 100), font, 1.5, (0, 0, 255), 2, cv2.LINE_AA)
...
clip1 = VideoFileClip("input_video/project_video.mp4").subclip(50, 51)
