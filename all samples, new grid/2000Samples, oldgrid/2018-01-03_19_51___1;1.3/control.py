...
abs_threshold = 80 # no of min detections to detect object
...
#    print("labels shape after: "+labels.shape)
    if (len(hot_windows_spat) > 5) and (len(hot_windows_hist) > 5) and (len(hot_windows_hog) > 5): # there have to be at least some detections in each filter
...
#    '''
    ...
    cv2.putText(draw_image, "hot windows found: "+str(len(hot_windows_hist)) , (150, 100), font, 1, (0, 0, 255), 2, cv2.LINE_AA)
