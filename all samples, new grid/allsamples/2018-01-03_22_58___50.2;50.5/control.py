...
dyn_lim = 0.25 # percentage of detections to locate object
abs_threshold = 15 # no of min detections to detect object
max_threshold = 30
...
#substart = 27; subend = 30 # critical situation

#substart = 0; subend = 50 # single car, black, far
...
sample_size = 8100 # for making the carlist for training and validation
...
#    print("labels shape after: "+labels.shape)
    if (len(hot_windows_spat) > 2) and (len(hot_windows_hist) > 2) and (len(hot_windows_hog) > 2): # there have to be at least some detections in each filter
